...
(define (merge comp list1 list2)
  (cond ((null? list1) list2)
        ((null? list2) list1)
        ((comp (car list1) (car list2)) (append (list (car list1))
                                                (merge comp (cdr list1) list2)))
        (else (append (list (car list2))
...
(define (patterns)
  (define (helper perm_list)
      (cond ((not (member? 'under (car perm_list))) 
                  (define perm_list (add_item perm_list))
                  (cons-stream (append (car perm_list) (list 'under)) 
                               (helper (cdr perm_list))))
            ((not (member? 'over (car perm_list)))
                  (define perm_list (add_item perm_list))
                  (cons-stream (append (car perm_list) (list 'over))
                               (helper (cdr perm_list))))
            (else (define perm_list (add_item perm_list))
                  (cons-stream (append (car perm_list) (list 'under))
                               (cons-stream (append (car perm_list) (list 'over)) 
                                            (helper (cdr perm_list))))) ))
  (define (add_item perm_list)
      (append perm_list (list (append (car perm_list) (list 'under))
                                      (append (car perm_list) (list 'over))))
  )
  (define s1 (helper (list (list 'over))))
  (define s2 (helper (list (list 'under))))
  (interleave s1 s2)
)
...
(define (tree-sums tree)
  (if (null? (children tree))
    (list (entry tree))
    (map (lambda (x) (+ (entry tree) x)) 
      (flatten (map (lambda (child) (tree-sums child)) (children tree))))
    )
  )
