...
'(1 (2 false . (3 . 989)))
; expect (1 (2 false 3 . 989))

(list 'cs '61a 'proj '4)
; expect (cs 61a proj 4)
...
(define six (list 1 2 3 4 5 6))
; expect six

(car (cdr (cdr (cdr (cdr six)))))
; expect 5
...
(define (filter f lst)
  (cond ((null? lst) '())
        ((f (car lst)) (append (list (car lst)) (filter f (cdr lst))))
        (else (filter f (cdr lst))))
)
(define (no-three num)
        (not (= num 3)))
(filter no-three (list 1 2 3 4))
; expect (1 2 4)
...
(all-satisfies '(1 2 -1 4) greater-than-zero?)
; expect #f
...
(stream-cdr finite)
; expect (0 . #[promise (not forced)])

(stream-car (stream-cdr finite))
; expect 0

(stream-car (stream-cdr (stream-cdr (stream-cdr finite))))
; expect 4
      ...
      (if (equal? b the-empty-stream)
          ...
          (cons-stream (+ (stream-car a) (stream-car b))
                       (stream-add (stream-cdr a)
                                   (stream-cdr b)) )) ))
(define result (stream-add finite nats))
(stream-car (stream-cdr result))
; expect 2

(stream-car (stream-cdr (stream-cdr (stream-cdr result))))
; expect 8
...
(stream-cdr (stream-cdr fib))
; expect (2 . #[promise (not forced)])

(stream-car (stream-cdr fib))
; expect 1

(stream-car (stream-cdr (stream-cdr (stream-cdr fib))))
; expect 3

(stream-car (stream-cdr (stream-cdr (stream-cdr (stream-cdr 
            (stream-cdr (stream-cdr (stream-cdr fib))))))))
; expect 21
...
(define (exp b n)
  (define (helper b n result)
    (if (= n 0)
      result
      (helper b (- n 1) (* b result))))
  (helper b n 1)
)
