...
def do_cons_stream_form(vals, env):
    ...
    check_form(vals, 2, 2)
    "*** YOUR CODE HERE ***"
    return Stream(scheme_eval(vals.first, env), vals.second, env), env
...
class Thunk(LambdaProcedure):
    ...
    def get_actual_value(self):
        val, env = self.apply(nil, self.env)
        return scheme_eval(val, self.env)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            lamb = Pair(target.second, vals.second)
            new_lamb, new_env = do_lambda_form(lamb, env)
            env.define(target.first, new_lamb)
            return target.first, None
        else:
...
def do_lambda_form(vals, env, function_type=LambdaProcedure):
    ...
    if len(vals.second) == 1:
        return function_type(formals, vals.second.first, env), env
    else:
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            "*** YOUR CODE HERE ***"
            num_results = len(clause.second)
            if num_results == 0:
                return clause.first, env
            elif num_results == 1:
...
def check_formals(formals):
    ...
    for formal in formals:
        if not scheme_symbolp(formal) or formal == symbol_so_far:
            raise SchemeError
        symbol_so_far = formal
